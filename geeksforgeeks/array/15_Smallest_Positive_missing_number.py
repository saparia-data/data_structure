...
1) In Smallest Positive Number, we need to find Only Positive number, so no need to consider Negative numbers.

2) After Eliminating Negative Numbers, Try using Index referencing for this Question, as we need to find the Smallest Positive Missing Number
...
3. After doing the above operation, Traverse the array with initial Positve values again, and store the first occurunce of index, 
   ...
   (Because this means that index value was not present in the array)
...
def missingNumber(arr,n):
    ...
    #loop to fill all integers greater than 0 in list in it's (position-1)
    for i in range(n):
        if(arr[i] > 0):
            if(L[arr[i] - 1] != 1):
                L[arr[i] - 1] = 1
